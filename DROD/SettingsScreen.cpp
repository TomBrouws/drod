...
void CSettingsScreen::SetupMediaTab(CTabbedMenuWidget* pTabbedMenu)
{
	...
	CLabelWidget* pLabel = new CLabelWidget(TAG_TARSTUFF_ALPHA_VALUE_LABEL, X_TARSTUFF_ALPHA_VALUE_LABEL, Y_TARSTUFFALPHA,
		...
		F_Small, compat::to_wstring(BYTE(255 - MIN_TARSTUFF_ALPHA + 1)).c_str());
	...
	pLabel = new CLabelWidget(TAG_SOUNDEFF_VALUE_LABEL, X_SOUNDEFF_VOLUME_LABEL, Y_SOUNDEFF_VOLUME,
		CX_SLIDER_VALUE_LABEL, CY_SLIDER_VALUE_LABEL,
		F_Small, compat::to_wstring(DEFAULT_SOUND_VOLUME).c_str());
	pLabel->SetAlign(CLabelWidget::TA_CenterGroup);
	...
	pLabel = new CLabelWidget(TAG_VOICE_VALUE_LABEL, X_VOICES_VOLUME_LABEL, Y_VOICES_VOLUME,
		CX_SLIDER_VALUE_LABEL, CY_SLIDER_VALUE_LABEL,
		F_Small, compat::to_wstring(DEFAULT_VOICE_VOLUME).c_str());
	pLabel->SetAlign(CLabelWidget::TA_CenterGroup);
	pSoundFrame->AddWidget(pLabel);
	...
	pLabel = new CLabelWidget(TAG_MUSIC_VALUE_LABEL, X_MUSIC_VOLUME_LABEL, Y_MUSIC_VOLUME,
		CX_SLIDER_VALUE_LABEL, CY_SLIDER_VALUE_LABEL,
		F_Small, compat::to_wstring(DEFAULT_MUSIC_VOLUME).c_str());
	pLabel->SetAlign(CLabelWidget::TA_CenterGroup);
	pSoundFrame->AddWidget(pLabel);
	...
	const UINT dwTagNo)       //(in)   Widget that event applied to.
{
	switch (dwTagNo)
	{
		...
		case TAG_TARSTUFF_TRANSPARENCY:
		{
			...
			pLabel->SetText(compat::to_wstring(pSliderWidget->GetValue() + MIN_TARSTUFF_ALPHA).c_str());
			Paint();
		}
		break;
		case TAG_SOUNDEFF_VOLUME:
		{
			...
			pLabel->SetText(compat::to_wstring(pSliderWidget->GetValue()).c_str());
			Paint();
		}
		break;
		case TAG_VOICES_VOLUME:
...
#  define SETMISSING(name, value) if (!Settings.DoesVarExist(name)) Settings.SetVar(name, value);
	...
	SETMISSING(Settings::ResizableWindow, CScreen::bAllowWindowed);
	...
	CDbPlayer *pPlayer)  //(in)   Player data.
{
	...
	pLabel->SetText(compat::to_wstring(bytValue).c_str());
	...
	pLabel->SetText(compat::to_wstring(bytVolume).c_str());
